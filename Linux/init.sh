...
# [MariaDB](https://mariadb.org/download/#mariadb-repositories)
curl -JL https://mariadb.org/mariadb_release_signing_key.asc | sudo gpg --dearmor --yes -o /usr/share/keyrings/mariadb-archive-keyring.gpg
echo "deb [signed-by=/usr/share/keyrings/mariadb-archive-keyring.gpg] http://sfo1.mirrors.digitalocean.com/mariadb/repo/10.6/debian $(lsb_release -cs) main" | sudo tee /etc/apt/sources.list.d/mariadb.list > /dev/null
...
echo "deb [signed-by=/usr/share/keyrings/nodejs-archive-keyring.gpg] https://deb.nodesource.com/node_16.x $(lsb_release -cs) main" | sudo tee /etc/apt/sources.list.d/nodejs.list > /dev/null
echo "deb-src [signed-by=/usr/share/keyrings/nodejs-archive-keyring.gpg] https://deb.nodesource.com/node_16.x $(lsb_release -cs) main" | sudo tee -a /etc/apt/sources.list.d/nodejs.list > /dev/null
